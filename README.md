...
В работе рассматривается задача согласования параметров поперечного движения заряженного пучка перед инжекцией в синхротрон. Цель исследования состоит в построении согласующей магнитной секции, обеспечивающей преобразование начальных параметров Твисса $(\alpha,\beta)$ в заданные выходные значения без изменения эмиттанса пучка. В качестве базовой конфигурации выбрана система вида Д--К--Д--К--Д--К--Д, где Д обозначает дрейфовый промежуток переменной длины, а К --- квадрупольную линзу. Основная идея решения заключается в том, что дрейфовые промежутки используются не только как пассивные участки транспортировки, но и как средство управляемого искажения параметров Твисса, после чего квадрупольные линзы выполняют сжатие или перераспределение фазового эллипса в требуемой плоскости. За счёт чередования дрейфов и трёх квадруполей удаётся поэтапно довести параметры пучка до целевых значений по обеим поперечным плоскостям. Особое внимание уделено различию между моделированием квадруполя в приближениях тонкой и толстой линзы, а также алгоритму численного подбора длин дрейфов и фокусирующих сил квадруполей.

Программные коды, с помощью которых совершаются точные рассчеты изменения параметров пучка, а также функции, реализующие отрисовку его поведения после прохождения участков загружены в репозиторий $github$.

\vspace{0.5cm}

\section{Введение}

Одной из ключевых задач физики ускорителей является подготовка пучка к инжекции в ускоряющую структуру. На этапе транспортировки пучок проходит через систему магнитных элементов, предназначенных для управления его поперечными характеристиками. Для эффективного ввода в синхротрон необходимо обеспечить совпадение параметров пучка с параметрами приемного канала ускорителя.

В рамках линейной поперечной динамики состояние пучка в каждой из двух поперечных плоскостей удобно описывается параметрами Твисса $(\alpha,\beta,\gamma)$ и эмиттансом $\varepsilon$. Величина эмиттанса характеризует площадь фазового эллипса и в идеализированной линейной гамильтоновой системе должна сохраняться. Следовательно, практическая задача согласования сводится не к изменению фазового объёма, а к преобразованию формы, наклона и масштаба фазового эллипса.

В данной работе исследуется проектирование согласующей секции, состоящей из трёх квадрупольных линз и четырёх дрейфовых промежутков. Такая структура выбрана как минимально достаточная для независимого управления параметрами в двух поперечных плоскостях. Ключевая идея состоит в том, что дрейфы используются для целенаправленного изменения параметров Твисса, а квадруполи --- для последующей компенсации и сжатия пучка в нужной плоскости. Тем самым управление параметрами осуществляется не отдельным элементом, а всей последовательностью Д--К--Д--К--Д--К--Д как единой оптической системой.
